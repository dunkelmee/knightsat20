...
export const AuditLogTab: React.FC = () => {
  ...
  if (isLoading) {
    return <div className="text-center py-10 text-xs text-on-surface-variant">Loading activity…</div>;
  }
  ...
  return (
    <div className="bg-surface-container-lowest rounded p-4 sm:p-5 border border-outline-variant/30 shadow-soft space-y-3">
      ...
      {logs.length === 0 ? (
        <p className="text-xs text-on-surface-variant text-center py-6">No actions recorded yet.</p>
      ) : (
        <div className="divide-y divide-outline-variant/20">
          {logs.map((log) => (
            <div key={log.id} className="py-2.5 flex items-start justify-between gap-3">
              <p className="text-xs text-on-surface">{log.description}</p>
              <span className="flex-shrink-0 text-[10px] text-outline whitespace-nowrap">
